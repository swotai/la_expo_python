...
def NAtoCSVAccu(inSpace, inGdb, inNetworkDataset, impedanceAttribute, accumulateAttributeName, inOrigins, inDestinations, outNALayerName, outFile):
    try:
        #Check out the Network Analyst extension license
        if arcpy.CheckExtension("Network") == "Available":
            arcpy.CheckOutExtension("Network")
        ...
        # SOMEHOW THIS WORKS, SO LETS KEEP IT THAT WAY
#        fields = ["OriginID", "DestinationID", "Name", "Total_Cost"]
        fields = ["OriginID", "DestinationID", "Name", "Total_Cost", "Total_Length", "Total_dps"]
        for lyr in arcpy.mapping.ListLayers(outNALayer):
            if lyr.name == linesLayerName:
                with open(outFile, 'w') as f:
                    #f.write(','.join(fields)+'\n') # csv headers
